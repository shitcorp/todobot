...
module.exports = {
    run: async (client, message, args, _level) => {
        if (!args[0]) 
            return message.channel.send(client.error(`You forgot to give a tag.`));
        if (!args[1]) 
            return message.channel.send(client.error(`You forgot to give a description for your tag.`));
        ...
        Config.find({ _id: message.guild.id }).then(res => {
            ...
            Config.updateOne({ _id: message.guild.id }, res[0], function(err, _affected, _resp) {
                if(err)
                    console.error(err);
                else {
