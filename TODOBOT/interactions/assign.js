...
module.exports = {
  ...
  run: async (client, interaction) => {
    ...
    if (!interaction.conf)
      return interaction.errorDisplay(messages.addbottoguild["en"]);
    let lang = interaction.conf
      ? interaction.conf.lang
        ? interaction.conf.lang
        : "en"
      : "en";
    ...
    if (user.bot === true)
      return interaction.errorDisplay(messages.cannotassignbots[lang]);

    let id;
    for (index in interaction.data.options)
      if (interaction.data.options[index].name === "id")
        id = interaction.data.options[index].value;
