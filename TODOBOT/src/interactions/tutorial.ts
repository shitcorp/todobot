...
export default {
    ...
    run: async (client: MyClient, interaction: Interaction) => {
        ...
        interaction.channel.send(tutorialEmbed).then(async (msg: Message) => {
            ...
            const Filter = (reaction, user) =>
                (reaction.emoji.name === beforeEmoji ||
                    reaction.emoji.name === nextEmoji ||
                    reaction.emoji.name === stopEmoji) &&
                user.id === interaction.member.user.id &&
                user.id !== client.user.id
            const otherFilter = (reaction, user) =>
                (reaction.emoji.name !== beforeEmoji &&
                    reaction.emoji.name !== nextEmoji &&
                    reaction.emoji.name !== stopEmoji) ||
                (user.id !== interaction.member.id && user.id !== client.user.id)
            ...
            update.on('collect', async (r, u) => {
                ...
                if (page === 2) {
                    tutorialEmbed.setTitle(pages[page - 1]).setDescription(`
                ...
                if (page === 3) {
                    tutorialEmbed
                        ...
                        .setDescription(
                            `To create a todo simply run the \`/todo\` command and submit at least a title. Once the todo is submitted it will be posted in your preconfigured todochannel. From there on everything else is done with reactions.
                        
                        **Note** To submit multiple tasks within a TODO, simply seperate them with a semicolon (;).
                        `,
                        )
                        .setImage(
                            'https://raw.githubusercontent.com/shitcorp/TODOBOT/develop/assets/todo_cmd_demo.gif',
                        )
                        .setThumbnail(client.user.displayAvatarURL())
                    tutorialEmbed.fields = []
                }
                if (page === 4) {
                    tutorialEmbed
                        ...
                        .setImage(
                            'https://cdn.discordapp.com/attachments/822998851848765440/822999581767630869/tagcmd_demo.gif',
                        )
                    tutorialEmbed.fields = []
                }
                if (page === 5) {
                    tutorialEmbed
                        ...
                        .setImage(
                            'https://cdn.discordapp.com/attachments/823001120590528532/823002155983568896/vars_demo.gif',
                        )
                }
                if (page === 6) {
                    tutorialEmbed.setTitle(pages[page - 1]).setDescription(`
