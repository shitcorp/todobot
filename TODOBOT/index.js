...
const agenda = new Agenda({
  db: {
    address:
      process.env.MONGO_CONNECTION ?? "mongodb://localhost:27017/todobot",
    options: {
...
(async function init() {
   ...
   */
  (await readdir(__dirname + "/commands/")).forEach((category) =>
  ...
  (await readdir(__dirname + "/interactions/")).forEach((file) => {
    ...
    client.interactions.set(
      interactionName,
      require(__dirname + "/interactions/" + file)
    );
  });
  ...
  client.ws.on(
    "INTERACTION_CREATE",
    async (raw_interaction) => await handle(client, raw_interaction)
  );
})();
